      ...
      switch (data["status"]) {
        case 1:
          if (!data.data["logged_in"]) {
            ...
            return window.location.href = window.baseurl + "/login";
          }
      }
    });
  };
  ...
  this.apiNotify = function(data, redirect) {
    ...
    if (redirect && data.status === 1) {
      return setTimeout((function() {
        return window.location = window.baseurl + redirect;
      }), 1000);
    }
  };
    ...
    return apiCall("GET", "/api/user/logout").done(function(data) {
      switch (data['status']) {
        case 1:
          ...
          return document.location.href = window.baseurl + "/";
        case 0:
          ga('send', 'event', 'Authentication', 'LogOut', 'Failure::' + data.message);
          return document.location.href = window.baseurl + "/login";
      }
    });
  };
